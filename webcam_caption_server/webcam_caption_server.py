...
print("Using GPU:", 'CUDAExecutionProvider' in ort.get_available_providers())
...
def generate_caption(enable_face_check, caption_prompt=None):
    ...
    if enable_face_check:
        ...
        logging.info("Face check disabled - using user mode")
    
    # Choose prompt based on recognition if using default
    if caption_prompt is None or caption_prompt.strip() == '':
        if enable_face_check and not is_known:
            ...
            logging.info("Using Default (user) prompt")
    else:
